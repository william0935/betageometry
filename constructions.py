...
class Canva:
    ...
    def apply_llm_construction(self, call: str):
        """Run a construction written as text, e.g. "midpoint(A, B)".

        Convenience wrapper for exploring a model's suggestion by hand. Parsing and
        validation live in `rabbits`, so there is exactly one definition of what a
        construction call may say; the solver's search loop goes through the same code.

        Returns ``(None, [])`` if the call cannot be parsed or does not apply, matching
        what the construction methods themselves return on failure.
        """
        from rabbits import ConstructionError, apply_construction, parse_construction

        try:
            name, points = parse_construction(call, self.points)
        except ConstructionError:
            return None, []

        new_points, relations = apply_construction(self, name, points, self.points)
        if not new_points:
            return None, []
        return (new_points[0] if len(new_points) == 1 else new_points), relations
